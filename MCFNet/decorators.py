...
def image_to_tensorboard1(static_name=None, pre_process=None):
    """
        Create decorator to write function output with tf.summary.image.
        Wrapped function should return
            image - (batch_size, h, w)
    ...
    def decorator(func):
        ...
        def wrapper(self, *args, name=None, **kwargs):
            ...
            out = tmp = func(self, *args, **kwargs)
            # print('aaaa',name)
            # print('ccccc', type(tmp),len(tmp),tmp)
            for i in range(len(tmp)):
                if tmp[i].shape[-1] > 3:
                    if "x" in name[i]:
                        ...
                        # print("cw1111:", ch)
                        # print("cw2222:", name)
                        # print("cw3333:", self.channels["x"])
                        name[i] += str(self.channels["x"])
                    elif "y" in name[i]:
                        ch = self.channels["y"]
                        name[i] += str(self.channels["y"])
                    tmp2 = tf.concat(
                        [
                            tf.expand_dims(tmp[i][..., ch[0]], -1),
                            tf.expand_dims(tmp[i][..., ch[1]], -1),
                            tf.expand_dims(tmp[i][..., ch[2]], -1),
                        ],
                        3,
                    )
                else:
                ...
                if (
                    ...
                    write_image_to_summary(tmp2, self.tb_writer, name[i], pre_process)
                if self._save_images and name[i] is not None:
                    filename = self._image_dir + tf.constant(f"/{name[i]}.png")
                    print('ijn', tmp2.shape, filename)
                    write_image_to_png(tmp2, filename)
            return out
